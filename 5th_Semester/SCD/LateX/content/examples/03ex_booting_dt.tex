...
\section{KR and Exercises: Booting and Device Tree}

\subsection{Boot Process}

\begin{example2}{Boot ROM Functions}

\textbf{Question:} During the boot process, a sequence of several steps is followed. One of them is executed from the Boot ROM.

Of the following statements, check all that are true.

Wählen Sie eine oder mehrere Antworten:
\begin{itemize}
    \item The boot ROM loads the first user-definable program into on-chip RAM \textbf{\textcolor{red}{X}}
    \item The Boot ROM sets up access to the SD card. \textcolor{frog}{$\surd$}
    \item The Boot ROM is the first step that can be reprogrammed by the user. \textbf{\textcolor{red}{X}}
    \item The Boot ROM configures the external DDR SDRAM device. \textcolor{frog}{$\surd$}
\end{itemize}
...
\important{Problem type:} Understanding boot sequence and Boot ROM responsibilities

\textbf{Source:} Moodle Quiz Booting and Device Tree, Question 1
\end{example2}

\begin{example2}{Bootloader Functions}

\textbf{Question:} During the boot process, a sequence of several steps is followed. One of them is called the bootloader, often it is the open source 'u-boot' program.

Of the following statements, check all that are true.

Wählen Sie eine oder mehrere Antworten:
\begin{itemize}
    \item The FPGA fabric must be configured before the bootloader is executed. \textcolor{frog}{$\surd$}
    \item The bootloader can be used to access the FPGA fabric via the h2f\_axi\_bridge. \textcolor{frog}{$\surd$}
    \item The bootloader is the first software program that is executed from the SDRAM. \textbf{\textcolor{red}{X}}
    \item The bootloader offers no interaction with the user. \textbf{\textcolor{red}{X}}
\end{itemize}
...
The bootloader can be used to access the FPGA fabric via the h2f\_axi\_bridge.

\important{Problem type:} Understanding bootloader role and capabilities

\textbf{Source:} Moodle Quiz Booting and Device Tree, Question 2
\end{example2}

\subsection{Device Tree}

\begin{example2}{Device Tree Address Lookup}

\textbf{Question:} The device tree is a hierarchical representation of the programmed logic and hardware as seen on a memory map. It is used to pass information about the hardware to the linux kernel, e.g. which driver to use and at what address a certain device can be accessed.

In the following image, a list of memory ranges on a bus is given as it appears in the device tree source.

%%\includegraphics[width=\linewidth]{ex_device_tree_ranges.png}
...
\important{Problem type:} Device tree interpretation and address mapping

\textbf{Source:} Moodle Quiz Booting and Device Tree, Question 3
\end{example2}
