...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Allgemeine Form: $x^{(k+1)} = -D^{-1}(L + R)x^{(k)} + D^{-1}b$
        \item Komponentenweise für $i=1,\ldots,n$:
        $$x_i^{(k+1)} = \frac{1}{a_{ii}}\left(b_i - \sum_{j=1,j\neq i}^n a_{ij}x_j^{(k)}\right)$$
        \item Alle Komponenten parallel berechnen
    \end{itemize}

    \item Gauss-Seidel-Verfahren:
    \begin{itemize}
        \item Allgemeine Form: $x^{(k+1)} = -(D+L)^{-1}Rx^{(k)} + (D+L)^{-1}b$
        \item Komponentenweise für $i=1,\ldots,n$:
        $$x_i^{(k+1)} = \frac{1}{a_{ii}}\left(b_i - \sum_{j=1}^{i-1} a_{ij}x_j^{(k+1)} - \sum_{j=i+1}^n a_{ij}x_j^{(k)}\right)$$
        \item Komponenten sequentiell berechnen
    \end{itemize}
