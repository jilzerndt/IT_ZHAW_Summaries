...
\begin{tikzpicture}
  ...
  \node[anchor=north west] (list) at (0, 0) {
    \begin{minipage}{\textwidth}
      \begin{itemize}
        \item $\Omega =$ Grundgesamtheit: Alle statistischen Einheiten, über die Aussagen gewonnen werden sollen
        \item $n =$ Anzahl Objekte in der Stichprobe (Stichprobengrösse)
        \item $X =$ Stichprobenwerte
        \item $a =$ Ausprägungen: Mögliche Werte des Merkmals
        \item $h =$ Absolute Häufigkeit: Anzahl Vorkommen einer Ausprägung
        \item $f =$ Relative Häufigkeit: $f_i = \frac{h_i}{n}$
        \item $H =$ Kumulative Absolute Häufigkeit: $H = \sum_{i=1}^n h_i$
        \item $F =$ Kumulative Relative Häufigkeit: $F = \sum_{i=1}^m f_i$
      \end{itemize}
    \end{minipage}
  };
