...
void OnCollisionEnter(Collision collision) {
    ...
    // Projektion der Geschwindigkeiten auf die Stoßnormale
    float v1n = Vector3.Dot(v1, normal);
    float v2n = Vector3.Dot(v2, normal);
    
    // Nur berechnen, wenn Objekte sich annähern
    if (v1n - v2n > 0) {
        // Geschwindigkeiten nach elastischem Stoß
        float u1n = ((m1 - m2) * v1n + 2 * m2 * v2n) / (m1 + m2);
        float u2n = ((m2 - m1) * v2n + 2 * m1 * v1n) / (m1 + m2);
        
        // Tangentiale Komponenten bleiben unverändert
